 ...
 * @param {Boolean} [mode=false] - Sets the mode to vector when true
 * @param {Boolean} [close=false] - Close the ends making the line continuous
 * @returns {Float32Array} New array with the calculated points that was added to the path
 */
function getCurvePoints(apoints, tension, numOfSeg, mode, close) {
	...
	if (mode) {
		...
		for (const vect of apoints) {


			points = points.concat(vectToPoint(vect))
	...
	if (mode) {
		...
		for (var i = 0; i < res.length; i += 2) {
			result.push(pointToVect(res[i], res[i + 1]))
		}
		return result
	}
...
// k = getCurvePoints([{ x: 5, y: 7 }, { x: 9, y: 10 }, { x: 10, y: 15 }], 0.5, 10000, true)
