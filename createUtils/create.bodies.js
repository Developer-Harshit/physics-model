...
function createParticle(mymode) {
  ...
  for (let i = 0; i < iter; i++) {
    ...
    if (mymode == "0") {
      aBody = new Ball(rWidth, rHeight, rSize, rColor);
    } else if (mymode == "1") {
      aBody = new Box(rWidth, rHeight, rSize * 2, rSize * 2, rColor);
    } else if (mymode == "2") {
      aBody = new Fluid(rWidth, rHeight, 7, rColor);
    } else if (mymode == "3") {
      aBody = new Ball(rWidth, rHeight, 12, rColor);

      rWidth = Math.random() * matterContainer.clientWidth;
      rHeight = Math.random() * matterContainer.clientHeight;
      var bBody = new Ball(rWidth, rHeight, 12, rColor);
